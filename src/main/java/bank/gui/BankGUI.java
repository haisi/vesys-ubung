...
public class BankGUI extends JFrame {
	private static final long serialVersionUID = 1663779102441297156L;
	...
	public BankGUI(BankDriver server) {
		...
		for (BankTest t : tests) {
			...
			m.addActionListener(e -> {
				try {
					t.runTests(BankGUI.this, bank,	currentAccountNumber());
					refreshDialog();
				} catch (Exception ex) {
					error(ex);
				}
		...
		item_gen.addActionListener(e -> generateAccounts());
	...
	public void generateAccounts() {
		...
		for (int i = 0; i < 5; i++) {
			try {
				number = bank.createAccount("Testaccount " + (i));
				Account acc = bank.getAccount(number);
				accounts.put(number, acc);
				acc.deposit(amount);
			} catch (Exception e) {
		...
		refreshDialog();
//		JOptionPane.showMessageDialog(this, "Example Accounts generated.", "Success", JOptionPane.INFORMATION_MESSAGE);
	}
	...
	static class AboutBox extends JDialog {
		...
		public AboutBox(Frame parent){
			...
			p_text.add(new JLabel("(c) D. Gruntz, 2001-2018", SwingConstants.CENTER));
	...
	static class AddAccountDialog extends JDialog {
		private static final long serialVersionUID = 4097926961865813715L;
	...
	static class TransferDialog extends JDialog {
		private static final long serialVersionUID = 4292725103733662666L;
